...
const EditableRoot = React.forwardRef<HTMLDivElement, EditableRootProps>((props, forwardedRef) => {
  const {
    defaultValue = '',
    value: valueProp,
  ...
  const composedRef = useComposedRefs(forwardedRef, (node) => setFormTrigger(node));
  ...
  (props, forwardedRef) => {
    ...
    return (
      <LabelPrimitive
        ...
        className={cn(
          "data-required:after:ml-0.5 data-required:after:text-destructive data-required:after:content-['*'] text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70",
          className,
        )}
      >
        {children}
...
const EditableArea = React.forwardRef<HTMLDivElement, EditableAreaProps>((props, forwardedRef) => {
  ...
  return (
    <AreaPrimitive
      ...
      ref={forwardedRef}
      className={cn(
        'data-disabled:cursor-not-allowed data-disabled:opacity-50 relative inline-block min-w-0',
        className,
      )}
    />
  );
  ...
  (props, forwardedRef) => {
    ...
    return (
      <PreviewPrimitive
        ...
        onKeyDown={onKeyDown}
        className={cn(
          'focus-visible:outline-hidden focus-visible:ring-ring data-disabled:cursor-not-allowed data-readonly:cursor-default data-empty:text-muted-foreground data-disabled:opacity-50 cursor-text truncate rounded-sm border border-transparent py-1 text-base focus-visible:ring-1 md:text-sm',
          className,
        )}
      >
...
const EditableInput = React.forwardRef<HTMLInputElement, EditableInputProps>(
  (props, forwardedRef) => {
    const { asChild, className, disabled, readOnly, required, maxLength, ...inputProps } = props;
    ...
    const onBlur = React.useCallback(
      ...
      [context.value, context.onSubmit, inputProps.onBlur, isDisabled, isReadOnly],
    );
    ...
    const onChange = React.useCallback(
      ...
      [context.onValueChange, inputProps.onChange, onAutosize, isDisabled, isReadOnly],
    );
    ...
    return (
      <InputPrimitive
        ...
        onKeyDown={onKeyDown}
        className={cn(
          'border-input shadow-xs file:text-foreground placeholder:text-muted-foreground focus-visible:outline-hidden focus-visible:ring-ring flex rounded-sm border bg-transparent py-1 text-base transition-colors file:border-0 file:bg-transparent file:text-sm file:font-medium focus-visible:ring-1 disabled:cursor-not-allowed disabled:opacity-50 md:text-sm',
...
const EditableTrigger = React.forwardRef<HTMLButtonElement, EditableTriggerProps>(
  (props, forwardedRef) => {
...
const EditableToolbar = React.forwardRef<HTMLDivElement, EditableToolbarProps>(
  (props, forwardedRef) => {
    const { asChild, className, orientation = 'horizontal', ...toolbarProps } = props;
...
const EditableSubmit = React.forwardRef<HTMLButtonElement, EditableSubmitProps>(
  (props, forwardedRef) => {
    ...
    const onClick = React.useCallback(
      ...
      [submitProps.onClick, context.onSubmit, context.value, context.disabled, context.readOnly],
    );
